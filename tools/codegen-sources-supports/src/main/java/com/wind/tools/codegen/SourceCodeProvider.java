...
public final class SourceCodeProvider {
    ...
    public SourceCodeProvider() {
        compilationUnitCaches = new ConcurrentHashMap<>(256);
        projectRoots = new ConcurrentHashMap<>(16);
        sourcesJarCaches = new ConcurrentHashMap<>(8);
    ...
    public <T extends TypeDeclaration> Optional<T> getTypeDeclaration(Class<?> clazz) {
        AssertUtils.notNull(clazz, "clazz must not null");
    ...
    public Optional<FieldDeclaration> getFieldDeclaration(Class<?> clazz, String fieldName) {
        return this.getTypeDeclaration(clazz).flatMap(declaration -> declaration
                ...
                .filter(item -> {
                    ...
                    Optional<VariableDeclarator> optional = fieldDeclaration.getVariables().getFirst();
                    AssertUtils.isTrue(optional.isPresent(), "fieldDeclaration.getVariables().getFirst() is null");
                    String name = optional.get().getName().asString();
    ...
    private Optional<CompilationUnit> getSourceCodeByClasses(Class<?> clazz, String classSourcePath) {
        Path mavenModuleRoot = CodeGenerationUtils.mavenModuleRoot(clazz);
